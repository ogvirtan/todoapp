...
def get_task(task_id):
    sql = """SELECT t.id, t.tila, t.task, t.body, t.user_id, u.username 
...
def get_task_by_user_and_status(user_id, status, page, page_size):
    sql = "SELECT id, task FROM tasks WHERE user_id = ? AND tila = ? LIMIT ? OFFSET ?"
    limit = page_size
    offset = page_size * (page - 1)
